    ...
    public sealed class Reflection
    {
        ...
        public Getters[] GetGetters(Type type, /*bool ShowReadOnlyProperties,*/ List<Type> IgnoreAttributes)
        {
            ...
            foreach (PropertyInfo p in props)
            {
                ...
                if (g != null)
                    getters.Add(new Getters { Getter = g, Name = p.Name, lcName = p.Name.ToLowerInvariant(), memberName = mName, ReadOnly = read_only, member = p });
            }
            ...
            foreach (var f in fi)
            {
                ...
                if (f.IsLiteral == false)
                {
                    ...
                    if (g != null)
                        getters.Add(new Getters { Getter = g, Name = f.Name, lcName = f.Name.ToLowerInvariant(), memberName = mName, ReadOnly = read_only, member = f });
                }
            }
            val = getters.ToArray();
