...
#region [   PROPERTY GET SET   ]
        ...
        internal Getters[] GetGetters(Type type, bool ShowReadOnlyProperties, List<Type> IgnoreAttributes)
        {
            ...
            foreach (PropertyInfo p in props)
            {
                ...
                if (g != null)
                    getters.Add(new Getters { Getter = g, Name = p.Name, lcName = p.Name.ToLower(), memberName = mName, member = p });
            }
            ...
            foreach (var f in fi)
            {
                ...
                if (f.IsLiteral == false)
                {
                    ...
                    if (g != null)
                        getters.Add(new Getters { Getter = g, Name = f.Name, lcName = f.Name.ToLower(), memberName = mName, member = f });
                }
            }
            val = getters.ToArray();
